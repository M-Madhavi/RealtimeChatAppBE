...
// method to create a post in chat
...
chatMessageSchema.statics.createPostInChatRoom = async function (chatRoomId, message, postedByUser) {
  try {
    ...
    const aggregate = await this.aggregate([
      ...
      { $match: { _id: post._id } },
      // joining on another table called users, and 
      // get  a user whose _id = postedByUser
      {
        $lookup: {
          from: 'users',
      ...
      { $unwind: '$postedByUser' },
      //  joining on another table called chatrooms, and 
      // getting a chatroom whose _id = chatRoomId
      {
        $lookup: {
          from: 'chatrooms',
...
chatMessageSchema.statics.markMessageRead = async function (chatRoomId, currentUserOnlineId) {
  try {
    return this.updateMany(
      ...
      {
        //to mark multiple images as Read
        multi: true
      }
    );
  } catch (error) {
...
chatMessageSchema.statics.getRecentConversation = async function (chatRoomIds, options, currentUserOnlineId) {
  try {
    return this.aggregate([
      ...
      // joining on another table called users 
      {
        $lookup: {
          from: 'users',
